...
class SmartPlug:
    ...
    def get_state(self, socket):
        status = self.get_setting(socket)

        if socket >= 0:
            return status['setting'][0]['metadata']['value']
        else:
            return "Error"
    ...
    def get_setting(self, socket):
        data = {
            "command": "get_setting",
            "setting":[
                {
                    "uid": 0,
                    "name ":"DSP-{}-{}-{}".format(self.model, self.u_nr(), str(socket)),
                    "idx": (socket - 1),
                	"type": 16
                }
            ]
        ...
        return self.send_json(data, log="Sending get status")
